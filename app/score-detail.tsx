...
function BreakdownRow({
  ...
  return (
    <View className="flex-row items-center py-3 border-b border-dark/5">
      <View className="w-10 h-10 rounded-full bg-sage/10 items-center justify-center mr-3">
      ...
      <Text className="text-xs font-semibold text-sage">{weight}</Text>
    </View>
  );
}
    ...
    <SafeAreaView className="flex-1 bg-cream">
      ...
      >
        ...
        <View className="mx-5 mt-4">
          <View
            className="bg-sage rounded-2xl p-6 items-center"
            style={{
              shadowColor: "#8B9E7C",
        ...
        <View className="flex-row px-5 mt-5 gap-3">
          <View className="flex-1 bg-white rounded-2xl py-3 items-center">
          ...
          <View className="flex-1 bg-white rounded-2xl py-3 items-center">
            ...
            <Text className="text-xs text-dark/50">Days Active</Text>
          </View>
        </View>
                ...
                >
                  ...
                  <View className="flex-1">
                    <View className="flex-row items-center">
                      <Text className={`text-sm font-semibold ${isCurrentTier ? "text-sage" : "text-dark"}`}>
                        {t.label}
                      </Text>
                      {isCurrentTier && (
                        <View className="ml-2 bg-sage/10 px-2 py-0.5 rounded-full">
                          <Text className="text-xs text-sage font-semibold">You</Text>
                        </View>
                      )}
                    </View>
        ...
        <View className="px-5 mt-6">
          ...
          <View className="bg-white rounded-2xl p-4">
            <View className="flex-row items-start mb-3">
              <Ionicons name="scan-outline" size={18} color="#8B9E7C" />
              <Text className="text-sm text-dark/70 ml-2 flex-1">
                Scan more products to build your scan history
              </Text>
            </View>
            <View className="flex-row items-start mb-3">
              <Ionicons name="restaurant-outline" size={18} color="#8B9E7C" />
              <Text className="text-sm text-dark/70 ml-2 flex-1">
                Try clean recipes from the Explore tab
              </Text>
            </View>
            <View className="flex-row items-start">
              <Ionicons name="swap-horizontal-outline" size={18} color="#8B9E7C" />
              <Text className="text-sm text-dark/70 ml-2 flex-1">
